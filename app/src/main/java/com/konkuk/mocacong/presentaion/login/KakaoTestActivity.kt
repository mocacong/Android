...
//                        sendCode(
//                            KakaoRequest(
//                                email = user.kakaoAccount!!.email!!, platformId = user.id.toString()
//                            )
//                        )
                    }
                }
    ...
    private fun sendCode(req: KakaoRequest) {
        lifecycleScope.launch {
            val signInApi = RetrofitClient.create(SignInAPI::class.java)
            val response = signInApi.kakaoLoginTest(req)
